...
public class CustomMappingBuilder {
    ...
    static XContentBuilder buildMapping(Class clazz, String indexType, String idFieldName, String parentType) throws IOException {
        ...
        mapEntity(xContentBuilder, clazz, true, idFieldName, "", false, CustomFieldType.Auto, null);

        return xContentBuilder.endObject().endObject().endObject();
    }

    private static void mapEntity(XContentBuilder xContentBuilder, Class clazz, boolean isRootObject, String idFieldName,
                                  String nestedObjectFieldName, boolean nestedOrObjectField, CustomFieldType fieldType, Field fieldAnnotation) throws IOException {
        ...
        if (!isRootObject && (isAnyPropertyAnnotatedAsField(fields) || nestedOrObjectField)) {
            ...
            if (nestedOrObjectField && CustomFieldType.Nested == fieldType && fieldAnnotation.includeInParent()) {
        ...
        for (java.lang.reflect.Field field : fields) {
            ...
            CustomField singleField = field.getAnnotation(CustomField.class);
     ...
     * @throws IOException
     */
    private static void addSingleFieldMapping(XContentBuilder builder, java.lang.reflect.Field field, CustomField annotation, boolean nestedOrObjectField) throws IOException {
    ...
    private static void addFieldMappingParameters(XContentBuilder builder, Object annotation, boolean nestedOrObjectField) throws IOException {
        ...
        if (annotation instanceof CustomField) {
            // @Field
            CustomField fieldAnnotation = (CustomField) annotation;
...
//            type = fieldAnnotation.type();
        ...
        if (type != CustomFieldType.Auto) {
            builder.field(FIELD_TYPE, type.name().toLowerCase());
            if (type == CustomFieldType.BINARY) {
                builder.field("doc_values", "true");
            }
            if (type == CustomFieldType.Date && dateFormat != DateFormat.none) {
